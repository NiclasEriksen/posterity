...
log = logging.getLogger("db_exporter")
...
def load_all_videos_from_disk(media_path: str):
    ...
    for original, dupes in duplicates.items():
        og = session.query(Video).filter_by(video_id=original).first()
        if not og:
            log.warning(f"Video lists {original} as original, but it doesn't exist!")
            continue
        for duplicate in dupes:
            d = session.query(Video).filter_by(video_id=duplicate).first()
            if not d:
...
def export_to_dbfile(p: str):
    init_db()
    load_all_videos_from_disk(p)
    return os.path.join(os.path.dirname(os.path.realpath(__file__)), "db.sqlite3")
